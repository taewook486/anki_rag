...
- [ ] Qdrant 배포 방식: 로컬 in-memory vs Docker 컨테이너 vs Qdrant Cloud
- [ ] 임베딩 캐시: 재인덱싱 시 임베딩 재계산 방지 여부
- [ ] 멀티링구얼 쿼리: 한국어 쿼리로 영어 단어 검색 시 성능 검증 필요
- [ ] 청크 전략: 예문이 긴 경우 분할 여부
- [ ] 오디오 여러 개인 경우: 단어당 복수 발음 파일 처리 (ex. 영국식 + 미국식)
- [ ] playsound Windows 호환성: playsound 1.3은 Python 3.12에서 이슈 있을 수 있음 → pygame 대안 검토
