...
그런데 기존 Anki 앱은 단순 키워드 일치 검색만 지원하기 때문에, "포기하다라는 뜻의 구동사를 모두 보여줘"처럼 의미 기반으로 질문하는 것이 불가능하다. 유의어끼리 묶어서 학습하거나, AI가 맥락을 종합해서 설명해 주는 기능도 없다.

이러한 한계를 메우려고 RAG(Retrieval-Augmented Generation) 아키텍처를 적용하여, Anki 데이터를 벡터 DB에 저장하고 하이브리드 검색과 LLM 응답 생성을 결합한 지능형 학습 도우미를 설계하였다.
...
> **[업데이트]** 기말 시점 실측 인덱스 규모는 Qdrant 컬렉션 anki_rag 기준 **73,410 포인트**다. 원본 24,472 카드를 다중 필드 청킹(단어/뜻/예문/예문 한글 번역)으로 쪼갠 결과다. 컬렉션이 20,000 포인트를 넘어가면서 로컬 파일 모드는 비권장 경고가 뜨므로, 운영 단계에서는 Qdrant Docker 서버로 전환할 계획이다.
...
먼저, Anki 버전에 따라 DB 파일명이 다르다. 신규 버전은 collection.anki21, 구버전은 collection.anki2를 사용하므로 두 경우를 모두 처리해야 했다. 다음으로, 카드의 필드 값이 notes.flds 컬럼에 \(ASCII 31) 문자로 구분되어 하나의 문자열에 합쳐져 있어서, 노트 타입별 필드 순서를 col.models JSON에서 먼저 파악한 뒤 매핑이 필요했다. 마지막으로, 일부 필드에 HTML 태그가 섞여 있어 BeautifulSoup으로 태그를 제거하는 전처리가 필요했다.
...
> **[업데이트]** 기말 시점에는 코어 레이어에 두 모듈이 추가되었다. src/agent.py(412라인 RAG와 별개의 ReAct 루프 + 자기 교정), src/adaptive.py(쿼리 복잡도 분류 + 전략 라우터), src/graph.py(594라인 지식 그래프 빌드/탐색). 코어 레이어는 9개 모듈로 확장되었다.

### 3.2 데이터 처리 흐름

시스템의 데이터 흐름은 인덱싱 단계와 검색 단계로 나뉜다.

**인덱싱 흐름**: .apkg/.txt 파일을 파서가 Document 리스트로 변환하면 임베더가 각 Document의 텍스트(단어+뜻+예문)를 BGE-M3 모델로 Dense 벡터(1024차원)와 Sparse 벡터를 동시에 생성한다. 이후 인덱서가 두 종류의 벡터와 메타데이터를 Qdrant에 저장한다.

**검색 흐름**: 사용자 쿼리가 입력되면 임베더가 쿼리를 벡터로 바꾸고, 리트리버가 Dense 검색과 Sparse 검색을 각각 수행한 뒤 RRF(Reciprocal Rank Fusion)로 결과를 병합한다. 병합된 상위 K개 결과를 컨텍스트로 삼아 LLM이 자연어 답변을 생성한다.
...
### 3.3 Web 레이어
FastAPI로 REST API 서버를 구성하고, Streamlit으로 Web UI를 띄운다.
...
Streamlit UI는 검색 페이지, 채팅 페이지, 관리 페이지로 구성하였다. 검색 페이지에서는 소스별 필터를 두었고, 채팅 페이지에서는 RAG 기반 대화형 질의응답을 지원한다.
...
> Streamlit UI에는 "지식 그래프" 탭이 추가되어 Plotly 기반으로 인터랙티브하게 시각화한다.

## 4. 핵심 모듈 설계

### 4.1 임베딩 모델 선택: BGE-M3

임베딩 모델 선택 과정에서 여러 후보를 비교하였다. OpenAI의 text-embedding-3-small은 API 호출 비용이 발생하고, sentence-transformers의 all-MiniLM은 영어 전용이라 한국어 뜻 검색에 불리했다. BGE-M3(BAAI/bge-m3)를 선택한 이유는 세 가지다.

먼저, Dense 벡터와 Sparse 벡터를 한 번의 인코딩으로 동시에 생성할 수 있어서 별도의 sparse 모델(BM25 등)을 따로 운영할 필요가 없다. 다음으로, 다국어를 지원하므로 영어 단어와 한국어 뜻이 혼합된 데이터에 적합하다. 마지막으로, 로컬에서 실행 가능하여 API 비용이 들지 않는다.

Dense 벡터는 1024차원 float32이며 코사인 유사도로 비교한다. Sparse 벡터는 SPLADE 방식의 토큰 가중치로, 특정 키워드가 문서에 얼마나 중요한지를 나타낸다. CUDA GPU가 있으면 fp16을 켜고, CPU 환경에서는 자동으로 끄도록 처리하였다.
...
Dense 검색은 의미가 비슷한 문서를 잘 찾아내지만 정확한 단어 매칭에 약하고 Sparse 검색은 키워드 매칭에 강하지만 유의어를 놓치기 쉽다. 두 검색 방식을 결합하려고 RRF(Reciprocal Rank Fusion) 알고리즘을 적용하였다.

RRF의 핵심 수식은 다음과 같다:

> score(doc) = 1/(rank_dense + k) + 1/(rank_sparse + k), k=60

k값은 상위 랭크에 과도한 가중치가 몰리는 것을 막는 상수로, 관련 논문에서 제안한 기본값 60을 사용하였다. Dense와 Sparse 각각에서 top_k의 3배 후보를 먼저 검색한 뒤 RRF로 병합하여 최종 top_k를 선택하는 방식(fetch_multiplier=3)도 함께 적용하여, 한쪽 검색에서만 상위에 오른 결과가 누락되는 문제를 줄였다.

### 4.4 RAG 파이프라인과 LLM 프로바이더

RAG 파이프라인은 검색 결과를 구조화된 컨텍스트로 조합해 LLM에 전달하고, 답변을 생성하는 과정이다.

컨텍스트 품질을 관리하려고 세 가지 필터를 적용하였다. RRF 점수가 0.005 미만인 저품질 결과는 제외했고 전체 컨텍스트는 2,000자로 제한했으며 개별 예문은 150자에서 잘라낸다. 이렇게 한 까닭은 LLM에 지나치게 긴 컨텍스트를 넣으면 오히려 핵심 정보를 놓치는 현상(lost in the middle)이 발생할 수 있기 때문이다.

LLM 프로바이더는 교체 가능한 구조로 설계하였다. Anthropic API 키가 있으면 Claude를 쓰고 없으면 OpenAI 호환 API(GLM, OpenRouter 등)를 쓴다. 시스템 프롬프트에 Few-shot 예시 2개를 포함해 응답 형식을 안내했고 할루시네이션을 막으려고 "검색 결과에 없는 내용은 답변하지 말 것"이라는 지시를 포함하였다.

> **[업데이트]** 기말 시연 환경의 기본 LLM은 claude-sonnet-4-6이다. Protocol로 정의한 LLMProvider 인터페이스(generate, stream 메서드)에 따라 AnthropicProvider와 OpenAICompatibleProvider 두 구현체가 존재하며, create_provider() 함수에서 ANTHROPIC_API_KEY → LLM_API_KEY 순으로 우선순위를 결정한다. 운영자는 .env 파일 한 줄(ANTHROPIC_API_KEY)만 추가/제거하면 코드 변경 없이 프로바이더를 바꾼다.

## 5. 검색 성능 향상 설계

기본 RAG 구현 이후, 검색 품질을 높이려고 다음과 같은 개선을 설계하였다.
...
Dense 검색과 Sparse 검색에서 각각 top_k개만 가져오면, 한쪽에서는 순위가 낮았지만 다른 쪽에서는 높은 문서가 RRF 후보에 포함되지 못하는 문제가 있다. 이를 해소하려고 각 검색에서 top_k x 3개의 후보를 가져온 뒤 RRF를 적용하였다. 이 설정은 검색 시간을 약간 증가시키지만 recall이 눈에 띄게 개선되었다.

### 5.3 Adaptive RAG (쿼리 복잡도 기반 전략 분기)
모든 쿼리에 동일한 검색 전략을 적용하는 것은 비효율적이다. "abandon"처럼 단순한 단어 검색에도 Agent 루프를 돌리면 불필요한 LLM 호출이 발생한다. 그래서 쿼리 복잡도를 분류하고, 복잡도에 따라 검색 전략을 동적으로 선택하는 Adaptive RAG를 설계하였다.
...
분류기는 2단계로 동작한다. 1단계에서 정규식 패턴 매칭으로 빠르게 분류하고 판단이 어려운 경우에만 2단계에서 LLM을 호출한다. 이렇게 하면 대부분의 단순 쿼리에서 LLM 호출 비용을 줄여준다.
...
검색 결과의 품질을 보장하려고 두 가지 자가 평가 메커니즘을 도입하였다.
...
벡터 검색만으로는 단어 간의 구조적 관계를 파악하기 어렵다. 예를 들어 "abandon"과 "give up"이 유의어라는 사실, "abandonment"가 "abandon"의 파생어라는 사실은 벡터 유사도만으로는 안정되게 잡아내기 어렵다. 특히 "반의어를 찾아줘" 같은 질의에서는 벡터 유사도가 오히려 높게 나오는 역설이 발생한다(의미가 가까운 맥락에서 쓰이므로).

단어 간 관계를 명시적으로 저장하는 지식 그래프를 설계해 이 한계를 메웠다.
...
유의어와 파생어 관계는 파싱 단계에서 규칙 기반으로 추출하고, 공기 관계(CO_OCCURS)는 예문 데이터의 동시 출현 빈도로 생성할 계획이다.
...
12. **CO_OCCURS**: 같은 노트의 예문 내 동시 출현. 폭주를 막으려고 **문서당 상한**(max_cooccurrence_per_doc=10)과 **전역 100k 엣지 경고**를 설정.
13. **SAME_CATEGORY**: 동일 deck 소속 단어 묶음.

### 6.3 GraphRAG Fusion
검색 시 벡터 유사도 점수와 그래프 연결 정보를 결합하는 GraphRAG Fusion을 설계하였다. 벡터 검색으로 얻은 상위 후보에 대해 그래프에서 인접 노드(유의어, 파생어 등)를 추가로 가져와 컨텍스트를 보강한다. 그러면 "give up 관련 구동사"라는 질의에서 벡터 유사도로는 놓칠 수 있는 "call off", "back out" 등의 관련어도 함께 제시한다.
...
기본 RAG 파이프라인은 "검색 → 응답"의 단일 단계로 동작한다. 이 구조는 단순 질의에는 충분하지만, "비즈니스 영어에서 계약 해지를 표현하는 단어들을 난이도 순으로 정리해줘"처럼 여러 단계의 검색과 종합이 필요한 질문에는 한계가 있다. 이 한계를 메우기 위해 LLM이 도구를 거듭 호출하며 스스로 추론하는 Agent 구조를 도입한다.
...
이처럼 Agent는 여러 소스를 차례로 탐색해 결과를 종합하므로, 단일 RAG로는 풀기 어려운 복합 학습 질의에 대응한다.

**자기 교정 시나리오**:

검색 결과의 RRF 점수가 임계값(0.005) 미만이면 Agent가 쿼리를 동의어로 재작성하여 재검색을 시도한다. 앞서 설명한 Corrective RAG를 Agent 루프 안에서 자연스럽게 구현한 셈이다.
...
> **[업데이트]** 7.4절은 구현 완료되었으며, Adaptive Complex 분기 외에도 POST /api/agent 엔드포인트로 Agent를 직접 호출한다. 통합 응답에는 ReAct 스텝별 thought / tool / args / observation / retry_count 메타데이터가 포함되어 추론 과정을 추적한다.
...
이 중 특히 Adaptive RAG는 실제 효과가 크다고 판단하였다. 단순 단어 검색(전체 쿼리의 약 60-70% 추정)에서 불필요한 LLM 호출을 줄여 응답 속도와 비용을 동시에 개선하기 때문이다.
...
응답 스키마(AdaptiveResponse)에는 다음 메타데이터가 포함되어 운영자가 분류 결과를 추적한다.
...
create_provider() 함수는 환경변수를 순서대로 검사하여 첫 번째로 사용 가능한 프로바이더를 반환한다. 운영자는 코드 변경 없이 .env 한 줄 수정으로 모델·공급자를 바꾼다. 본 시연 환경에서는 claude-sonnet-4-6을 기본으로 한다.
...
SYSTEM_PROMPT의 "검색 결과에 없는 내용은 답변하지 말 것" 지시가 실제로 LLM 응답에 반영됨을 보여주는 증거다.
...
사용자는 답변의 어떤 부분이 어느 학습 덱에서 인출되었는지 즉시 확인한다.

## 12. 결론 및 향후 계획

본 설계서에서는 Anki 플래시카드 데이터를 활용한 RAG 기반 영어 학습 시스템의 전체 설계와 기말 구현 결과를 종합 기술하였다. BGE-M3의 Dense+Sparse 동시 임베딩, RRF 기반 하이브리드 검색, Adaptive RAG로 전략을 분기하는 구조, ReAct Agent로 복합 질의를 처리하는 흐름, 지식 그래프로 구조적 관계까지 검색하는 단계 — 모든 핵심 구성 요소가 통합 동작함을 확인하였다.

기말 프로젝트에서는 설계 완료 단계였던 지식 그래프(NetworkX 백엔드)를 실제 구현하여 v2.0으로 반영하였다. WordNet 기반 ANTONYM 자동 추출, 그래프 영속화(pickle+GraphML 이중화), CO_OCCURS 엣지 상한, AdaptiveRAG Complex 경로 Fusion 주입, FastAPI/Streamlit 인터페이스까지 전체 파이프라인을 통합하였다. Neo4j 백엔드로의 확장과 Agent 도구 세트 확장(학습 계획 수립, 복습 일정 생성 등 개인화 학습 지원)은 향후 개선 과제로 남긴다.

### 12.1 회고

본 프로젝트를 진행하며 얻은 핵심 학습은 세 가지다.

하나는 추상화 인터페이스의 힘이다. LLMProvider Protocol과 단일 AdaptiveRAG.query() 엔트리는 시연 단계에서 환경 변경 비용을 크게 줄였다. 시연 직전 LLM 공급자가 사용량 정책으로 throttle을 걸었을 때, 코드 변경 없이 .env 한 줄(ANTHROPIC_API_KEY)을 추가하여 5분 안에 복구하였다.

다음은 점진적 SPEC 분할이다. v1.1 → v1.2 → v1.3 → v2.0의 단계별 SPEC 분할은 각 단계의 회귀 테스트를 보존하면서 새 기능을 안전하게 통합하도록 도왔다. 모든 단계에서 이전 테스트가 통과하는 상태로 합치는 원칙을 지켰다.

마지막은 자료의 양보다 분류의 질이다. 73,410 포인트의 인덱스보다, 응답의 sources 필드 하나가 사용자 신뢰에 더 큰 영향을 미쳤다. "어디서 가져왔는가"를 명시하는 한 줄이 LLM의 자신만만한 환각보다 가치가 훨씬 크다.
