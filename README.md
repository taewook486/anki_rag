...
- **지식 그래프**: 단어 유의어·반의어·파생어 관계 그래프 + GraphRAG Fusion
...
- **오디오 재생**: 단어 발음 재생 지원
- **Web UI**: FastAPI + Streamlit 기반 검색·채팅·관리 인터페이스
...
├── graph.py            # 지식 그래프 (NetworkX MultiDiGraph + GraphRAG Fusion)
├── __main__.py         # CLI 진입점
├── api/
│   ├── main.py         # FastAPI 앱
│   └── routes/         # search / query / index / audio / cache / agent
└── web/
    └── app.py          # Streamlit UI
